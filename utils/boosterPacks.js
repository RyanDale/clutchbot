...
const getCard = async args => await Card.aggregate([
    { $match: args },
    { $sample: { size: 1 } }
]);
...
Card 10 – One random uncommon player from 102 or 103 cards – 50 or 51 pitchers, 52 batters (removing cards 13, 12 and 11 from the possibilities)
...
const series1PackBuilder = {
    ...
    async card14(pack) {
        ...
        if (pack[0].cardType === 'Pitcher') {
            kwargs.cardType = 'Batter';
        }
        return await await this.card15(kwargs);
    },
